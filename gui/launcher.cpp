    ...
    ui(new Ui::Launcher)
{
    ...
    Tab* sigma   = new Tab(SERVER,  "Sigma",   ui->sigmaSelected, 2.10);
    Tab* epsilon = new Tab(SERVER,  "Epsilon", ui->epsilonSelected, 2.38);
        ...
        if (tab->window == NULL)
        {
            switch (tab->type)
            {
                case HOME:
                    ...
                    break;
                case SERVER:
                    tab->window = new Server(ui->mainWidget, this, tab->name, tab->version);
                    break;
                case CONSOLE:
